  ...
  return (
    <div className="container mx-auto px-4 py-8 relative">
      ...
      <div className="flex items-center justify-center mb-6">
        ...
        <h3 className="text-xl text-gray-500 font-semibold">Recently Posted </h3>
      </div>
      <div className="mb-8">
          {!imageError ? (
            <img 
      ...
      <button
        ...
        className={`fixed bottom-[62px] right-4 bg-blue-500 text-white rounded-full p-4 shadow-lg z-20 transition-all duration-300 ${
          showFullButton ? 'w-auto px-6' : 'w-12 h-12'
        }`}
      >
        <div className="flex items-center justify-center">
          <FaPlus className={`${showFullButton ? 'mr-2' : ''}`} />
          <span className={`${showFullButton ? 'inline' : 'hidden'}`}>Post</span>
        </div>
      </button>
    </div>
  );
